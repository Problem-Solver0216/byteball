var BLACKBYTES_ASSET = require('millixcore/constants').BLACKBYTES_ASSET;
var balances = require('millixcore/balances');
...
function initWallet() {
	...
	function loadCompleteClient(showClient) {
		self._bMillixCoreLoaded = false; //"fix" : Looks like you are loading multiple copies of millix core, which is not supported. Running 'npm dedupe' might help.
		...
		angularJs.onload = function() {
			var millixJS = document.createElement('script');
			millixJS.src = 'millix.js';
			body.appendChild(millixJS);
			millixJS.onload = function() {
